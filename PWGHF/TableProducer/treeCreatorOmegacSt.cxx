...
DECLARE_SOA_TABLE(HfStChBars, "AOD", "HFSTCHBAR",
                  ...
                  hf_st_charmed_baryon::DecayChannel);
} // namespace o2::aod
...
struct HfTreeCreatorOmegacSt {
  ...
  std::shared_ptr<TH1> hCandidatesPrPi, hCandidatesV0Pi, hCandidatesCascPiOrKa;
  HistogramRegistry registry{
    "registry",
    {
      ...
      {"hMassOmegac", "inv. mass #Omega_c^{0};inv. mass (GeV/#it{c}^{2})", {HistType::kTH1D, {{400, 1.5, 3.}}}},
      {"hMassOmegacVsPt", "inv. mass #Omega_c^{0};inv. mass (GeV/#it{c}^{2});p_{T} (GeV/#it{c})", {HistType::kTH2D, {{400, 1.5, 3.}, {10, 0., 10.}}}},
  ...
  void init(InitContext const&)
  {
    ...
    hCandidatesCascPiOrKa = registry.add<TH1>("hCandidatesCascPiOrKa", "Casc-Pi/Ka candidates counter", {HistType::kTH1D, {axisCands}});
    setLabelHistoCands(hCandidatesPrPi);
    setLabelHistoCands(hCandidatesV0Pi);
    setLabelHistoCands(hCandidatesCascPiOrKa);
  }
      ...
      if (isOmegaC || isXiC) {
        ...
        if (daughters.size() == 2) {
          ...
          if (idxPionDaughter >= 0 && idxCascDaughter >= 0) {
             decayChannel = 0; // OmegaC -> Omega + Pi
          } else if (idxKaonDaughter >= 0 && idxCascDaughter >= 0) {
              decayChannel = 1; // OmegaC -> Omega + K
          }
          if (decayChannel != -1) { // here include two channels
            int idxDaughter = (decayChannel == 0) ? idxPionDaughter : idxKaonDaughter;
          ...
          if (((std::abs(bachelor.tpcNSigmaKa()) < maxNSigmaBachelor) || (std::abs(bachelor.tpcNSigmaPi()) < maxNSigmaBachelor)) &&
            ...
            for (auto trackId : groupedTrackIds) {
              ...
              if ((track.itsNCls() >= 4) &&
                ...
                hCandidatesCascPiOrKa->Fill(SVFitting::BeforeFit);
                try {
                  ...
                  LOG(info) << "Run time error found: " << error.what() << ". DCAFitterN for Casc-Pi/Ka cannot work, skipping the candidate.";
                  hCandidatesCascPiOrKa->Fill(SVFitting::Fail);
                  continue;
                }
                hCandidatesCascPiOrKa->Fill(SVFitting::FitOk);
              }
            }
          }
        }
